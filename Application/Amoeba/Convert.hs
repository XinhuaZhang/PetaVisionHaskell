...
main = do
  (filePath:folerPath:_) <- getArgs
  (tFilePathList, aFilePathList) <-
    (L.unzip . L.map split . L.lines) <$> readFile filePath
  ...
  MP.sequence_ $
    L.zipWith3
      (\i tFilePath aFilePath -> do
         ...
         plotImageRepa (folerPath </> "t" </> (show i L.++ ".png")) tImg
         plotImageRepa (folerPath </> "a" </> (show i L.++ ".png")) $
           labelImage tImg aImg)
      [1 ..]
      tFilePathList
      aFilePathList
  writeFile (folerPath </> "imageList.txt") . L.unlines $
    [ (folerPath </> "t" </> (show i L.++ ".png")) L.++ " " L.++
