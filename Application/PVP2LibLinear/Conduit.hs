...
concatConduit offset =   awaitForever
                           (yield . P.concat .
                            P.zipWith (\i x -> P.map (\(ind,val) -> (ind + i + 1,val)) x) offset . P.map parsePVPOutputData)
  where parsePVPOutputData :: PVPOutputData -> [(Int,Double)]
        parsePVPOutputData (PVP_NONSPIKING_ACT xs) = P.zipWith (\i v -> (i,v)) [1..] xs
