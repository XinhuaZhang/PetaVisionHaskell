...
main = do
  ...
  let sortedPairs = L.reverse . L.sortOn fst . V.toList $ V.zip cs kmeansModel
      xs =
        L.map
          (\x' ->
              let arr' = fromUnboxed (Z :. actNy :. actNx :. actNf) x'
              in L.map
              ...
              then let (Z :. ny' :. nx' :. nf' :. numDict) = extent w
                   ...
                   in parMap
                        ...
                        (sumS .
                         ...
                         L.zipWith
                           (\weight' y ->
                               let weightArr = repaArray2UArray3 weight'
                     ...
                     rseq
                     (\zs ->
                         let vec = fromUnboxed (Z :. (VU.length zs)) zs
                         in sumS $
                            traverse2
                              w
                              vec
