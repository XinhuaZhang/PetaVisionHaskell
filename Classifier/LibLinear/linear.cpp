...
void l2r_lr_fun::grad(double *w, double *g) {
  ...
  for (i = 0; i < w_size; i++) g[i] = w[i] + g[i];
}
...
void l2r_lr_fun::Hv(double *s, double *Hs) {
  ...
  for (i = 0; i < w_size; i++) Hs[i] = s[i] + Hs[i];
  delete[] wa;
}
...
void l2r_lr_fun::Xv(double *v, double *Xv) {
  ...
  for (i = 0; i < l; i++) Xv[i] = sparse_operator::dot(v, x[i]);
}
...
void l2r_lr_fun::XTv(double *v, double *XTv) {
  ...
  for (i = 0; i < l; i++) sparse_operator::axpy(v[i], x[i], XTv);
}
...
int l2r_l2_svc_fun::get_nr_variable(void) { return prob->n; }
...
void l2r_l2_svc_fun::subXTv(double *v, double *XTv) {
  ...
  for (i = 0; i < w_size; i++) XTv[i] = 0;
  for (i = 0; i < sizeI; i++) sparse_operator::axpy(v[i], x[I[i]], XTv);
}
...
class Solver_MCSVM_CS {
 ...
 private:
  void solve_sub_problem(double A_i, int yi, double C_yi, int active_i,
                         double *alpha_new);
...
Solver_MCSVM_CS::Solver_MCSVM_CS(const problem *prob, int nr_class,
                                 double *weighted_C, double eps, int max_iter) {
  this->w_size = prob->n;
  this->l = prob->l;
...
void Solver_MCSVM_CS::solve_sub_problem(double A_i, int yi, double C_yi,
                                        int active_i, double *alpha_new) {
  int r;
  double *D;
...
bool Solver_MCSVM_CS::be_shrunk(int i, int m, int yi, double alpha_i,
                                double minG) {
  double bound = 0;
...
void Solver_MCSVM_CS::Solve(double *w) {
  ...
  for (i = 0; i < l * nr_class; i++) alpha[i] = 0;

  for (i = 0; i < w_size * nr_class; i++) w[i] = 0;
  for (i = 0; i < l; i++) {
    for (m = 0; m < nr_class; m++) alpha_index[i * nr_class + m] = m;
  ...
  while (iter < max_iter) {
    ...
    for (s = 0; s < active_size; s++) {
      ...
      if (Ai > 0) {
        for (m = 0; m < active_size_i[i]; m++) G[m] = 1;
        ...
        while (xi->index != -1) {
          double *w_i = &w[(xi->index - 1) * nr_class];
          for (m = 0; m < nz_d; m++) w_i[d_ind[m]] += d_val[m] * xi->value;
          xi++;
        }
      }
    }
    ...
    if (stopping < eps_shrink) {
      if (stopping < eps && start_from_all == true)
        ...
        active_size = l;
        for (i = 0; i < l; i++) active_size_i[i] = nr_class;
        info("*");
  ...
  double v = 0;
  int nSV = 0;
  for (i = 0; i < w_size * nr_class; i++) v += w[i] * w[i];
  v = 0.5 * v;
  for (i = 0; i < l * nr_class; i++) {
    v += alpha[i];
    if (fabs(alpha[i]) > 0) nSV++;
  }
  for (i = 0; i < l; i++) v -= alpha[i * nr_class + (int)prob->y[i]];
...
static void solve_l2r_l1l2_svr(const problem *prob, double *w,
                               const parameter *param, int solver_type) {
  int l = prob->l;
  double C = param->C;
  double p = param->p;
  int w_size = prob->n;
  ...
  double Gnorm1_init =
      -1.0;  // Gnorm1_init is initialized at the first iteration
  ...
  while (iter < max_iter) {
    ...
    for (s = 0; s < active_size; s++) {
      ...
      beta[i] =
          min(max(beta[i] + d, -upper_bound[GETI(i)]), upper_bound[GETI(i)]);
      d = beta[i] - beta_old;

      if (d != 0) sparse_operator::axpy(d, xi, w);
    }
  ...
  v = 0.5 * v;
  for (i = 0; i < l; i++) {
    v += p * fabs(beta[i]) - y[i] * beta[i] +
         0.5 * lambda[GETI(i)] * beta[i] * beta[i];
    if (beta[i] != 0) nSV++;
  }
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp,
                       double Cn) {
  int l = prob->l;
  int w_size = prob->n;
  int i, s, iter = 0;
  ...
  while (iter < max_iter) {
    ...
    if (newton_iter <= l / 10) innereps = max(innereps_min, 0.1 * innereps);
  }
...
static void solve_l1r_l2_svc(problem *prob_col, double *w, double eps,
                             double Cp, double Cn) {
  int l = prob_col->l;
    ...
    for (s = 0; s < active_size; s++) {
      ...
      for (num_linesearch = 0; num_linesearch < max_num_linesearch;
        ...
        if (num_linesearch == 0) {
          ...
          while (x->index != -1) {
            int ind = x->index - 1;
            if (b[ind] > 0) loss_old += C[GETI(ind)] * b[ind] * b[ind];
            double b_new = b[ind] + d_diff * x->value;
            b[ind] = b_new;
            if (b_new > 0) loss_new += C[GETI(ind)] * b_new * b_new;
            x++;
          }
        } else {
          loss_new = 0;
...
static void solve_l1r_lr(const problem *prob_col, double *w, double eps,
  ...
  for (j = 0; j < w_size; j++) {
    ...
    while (x->index != -1) {
      ...
      double val = x->value;
      exp_wTx[ind] += w[j] * val;
      if (y[ind] == -1) xjneg_sum[j] += C[GETI(ind)] * val;
      x++;
    }
  }
  for (j = 0; j < l; j++) {
  ...
  while (newton_iter < max_newton_iter) {
    ...
    for (j = 0; j < w_size; j++) {
      ...
      if (wpd[j] != 0) w_norm_new += fabs(wpd[j]);
    }
    delta += (w_norm_new - w_norm);

    negsum_xTd = 0;
    for (int i = 0; i < l; i++)
      if (y[i] == -1) negsum_xTd += C[GETI(i)] * xTd[i];
    ...
    for (num_linesearch = 0; num_linesearch < max_num_linesearch;
      ...
      for (int i = 0; i < l; i++) {
        ...
        cond +=
            C[GETI(i)] * log((1 + exp_wTx_new[i]) / (exp_xTd + exp_wTx_new[i]));
      }

      if (cond <= 0) {
        w_norm = w_norm_new;
        for (j = 0; j < w_size; j++) w[j] = wpd[j];
        for (int i = 0; i < l; i++) {
    ...
    if (num_linesearch >= max_num_linesearch) {
      ...
      for (int i = 0; i < l; i++) exp_wTx[i] = exp(exp_wTx[i]);
    }
...
static void transpose(const problem *prob, feature_node **x_space_ret,
                      problem *prob_col) {
  int i;
  int l = prob->l;
  int n = prob->n;
  ...
  for (i = 0; i < l; i++) prob_col->y[i] = prob->y[i];

  for (i = 0; i < n + 1; i++) col_ptr[i] = 0;
  for (i = 0; i < l; i++) {
  ...
  for (i = 1; i < n + 1; i++) col_ptr[i] += col_ptr[i - 1] + 1;

  x_space = new feature_node[nnz + n];
  for (i = 0; i < n; i++) prob_col->x[i] = &x_space[col_ptr[i]];
  ...
  for (i = 0; i < n; i++) x_space[col_ptr[i]].index = -1;
...
model *train(const problem *prob, const parameter *param) {
  ...
  if (check_regression_model(model_)) {
    ...
    for (i = 0; i < w_size; i++) model_->w[i] = 0;
    model_->nr_class = 2;
    model_->label = NULL;
    ...
    for (i = 0; i < nr_class; i++) model_->label[i] = label[i];
    ...
    if (param->solver_type == MCSVM_CS) {
      ...
      for (i = 0; i < nr_class; i++)
        for (j = start[i]; j < start[i] + count[i]; j++) sub_prob.y[j] = i;
      ...
      if (nr_class == 2) {
        ...
        if (param->init_sol != NULL)
          for (i = 0; i < w_size; i++) model_->w[i] = param->init_sol[i];
        else
          for (i = 0; i < w_size; i++) model_->w[i] = 0;
...
#pragma omp parallel for private(i, j, k)
        for (i = 0; i < nr_class; i++) {
          ...
          for (; k < si; k++) sub_prob_omp.y[k] = -1;

          for (; k < ei; k++) sub_prob_omp.y[k] = +1;

          for (; k < sub_prob_omp.l; k++) sub_prob_omp.y[k] = -1;
          ...
          for (j = 0; j < w_size; j++) model_->w[j * nr_class + i] = w[j];
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold,
                      double *target) {
  int i;
  int *fold_start;
  int l = prob->l;
...
void find_parameter_C(const problem *prob, const parameter *param, int nr_fold,
                      double start_C, double max_C, double *best_C,
                      double *best_rate) {
  // variables for CV
  int i;
  int *fold_start;
  ...
  for (i = 0; i < nr_fold; i++) prev_w[i] = NULL;
  ...
  while (param1.C <= max_C) {
    ...
    for (i = 0; i < nr_fold; i++) {
      ...
      if (prev_w[i] == NULL) {
        ...
        for (j = 0; j < total_w_size; j++) prev_w[i][j] = submodel->w[j];
      } else if (num_unchanged_w >= 0) {
        double norm_w_diff = 0;
        for (j = 0; j < total_w_size; j++) {
          norm_w_diff +=
              (submodel->w[j] - prev_w[i][j]) * (submodel->w[j] - prev_w[i][j]);
...
double predict_probability(const struct model *model_,
  ...
  if (check_probability_model(model_)) {
    ...
    if (nr_class == 2)  // for binary classification
      ...
      double sum = 0;
      for (i = 0; i < nr_class; i++) sum += prob_estimates[i];
...
int save_model(const char *model_file_name, const struct model *model_) {
  ...
  if (model_->label) {
    fprintf(fp, "label");
    for (i = 0; i < model_->nr_class; i++) fprintf(fp, " %d", model_->label[i]);
    fprintf(fp, "\n");
  }
  ...
  for (i = 0; i < w_size; i++) {
    int j;
    for (j = 0; j < nr_w; j++) fprintf(fp, "%.16g ", model_->w[i * nr_w + j]);
    fprintf(fp, "\n");
  }
...
struct model *load_model(const char *model_file_name) {
  ...
  while (1) {
    ...
    if (strcmp(cmd, "solver_type") == 0) {
      ...
      int nr_class = model_->nr_class;
      model_->label = Malloc(int, nr_class);
      for (int i = 0; i < nr_class; i++) FSCANF(fp, "%d", &model_->label[i]);
    } else {
  ...
  for (i = 0; i < w_size; i++) {
    int j;
    for (j = 0; j < nr_w; j++) FSCANF(fp, "%lf ", &model_->w[i * nr_w + j]);
    if (fscanf(fp, "\n") != 0) {
      fprintf(stderr, "ERROR: fscanf failed to read the model\n");
      EXIT_LOAD_MODEL()
    }
...
int get_nr_feature(const model *model_) { return model_->nr_feature; }

int get_nr_class(const model *model_) { return model_->nr_class; }

void get_labels(const model *model_, int *label) {
  if (model_->label != NULL)
    for (int i = 0; i < model_->nr_class; i++) label[i] = model_->label[i];
}

// use inline here for better performance (around 20% faster than the non-inline
// one)
static inline double get_w_value(const struct model *model_, int idx,
                                 int label_idx) {
  int nr_class = model_->nr_class;
  int solver_type = model_->param.solver_type;
  const double *w = model_->w;

  if (idx < 0 || idx > model_->nr_feature) return 0;
  if (check_regression_model(model_))
    return w[idx];
  else {
    if (label_idx < 0 || label_idx >= nr_class) return 0;
...
double get_decfun_coef(const struct model *model_, int feat_idx,
                       int label_idx) {
  if (feat_idx > model_->nr_feature) return 0;
...
void free_model_content(struct model *model_ptr) {
  if (model_ptr->w != NULL) free(model_ptr->w);
  if (model_ptr->label != NULL) free(model_ptr->label);
}
...
void destroy_param(parameter *param) {
  if (param->weight_label != NULL) free(param->weight_label);
  if (param->weight != NULL) free(param->weight);
  if (param->init_sol != NULL) free(param->init_sol);
}
...
const char *check_parameter(const problem *prob, const parameter *param) {
  ...
  if (param->solver_type != L2R_LR &&
      ...
      param->solver_type != MCSVM_CS && param->solver_type != L1R_L2LOSS_SVC &&
      param->solver_type != L1R_LR && param->solver_type != L2R_LR_DUAL &&
      param->solver_type != L2R_L2LOSS_SVR &&
      param->solver_type != L2R_L2LOSS_SVR_DUAL &&
      param->solver_type != L2R_L1LOSS_SVR_DUAL)
    return "unknown solver type";

  if (param->init_sol != NULL && param->solver_type != L2R_LR &&
      param->solver_type != L2R_L2LOSS_SVC)
    return "Initial-solution specification supported only for solver L2R_LR "
           "and L2R_L2LOSS_SVC";
